# 🚀 Data Science Project Lab

A curated collection of practical Data Science, Machine Learning, NLP, recommendation, forecasting, and analytics projects.

## 🌟 Projects

### ✅ Built

| Project | Focus | Status |
|---|---|---|
| 💳 [Loan Approval Prediction](../) | Classification + preprocessing | ✅ Built |
| 🌦️ [Weather Trend & Forecasting](weather-trend-forecasting/) | Time series + regression | ✅ Built |

### 🚧 Portfolio Roadmap

| Project | Focus | Level |
|---|---|---|
| 🏠 [House Price Intelligence](HOUSE_PRICE_INTELLIGENCE.md) | Regression + EDA + explainability | Intermediate |
| 💳 [FraudShield](FRAUDSHIELD.md) | Fraud detection + imbalanced data | Intermediate |
| 🛒 [CustomerIQ](CUSTOMERIQ.md) | RFM + K-Means clustering | Intermediate |
| 💬 [SentimentAI](SENTIMENTAI.md) | NLP + text classification | Intermediate |
| 📈 [Demand Forecasting](DEMAND_FORECASTING.md) | Time series + backtesting | Intermediate |
| 🎬 [Movie Recommendation Engine](MOVIE_RECOMMENDER.md) | Recommendation systems | Intermediate |
| 🚕 [Taxi Demand Predictor](TAXI_DEMAND_PREDICTOR.md) | Forecasting + geospatial analysis | Advanced |
| 📊 [E-Commerce Analytics Dashboard](ECOMMERCE_ANALYTICS.md) | SQL + KPIs + BI | Intermediate |

## 🧰 Core Stack

Python · Pandas · NumPy · Scikit-learn · Matplotlib · Seaborn · SQL · Jupyter · Streamlit · Git · GitHub

## 🎯 Portfolio Direction

The goal is to demonstrate an end-to-end Data Science workflow: data preparation, exploratory analysis, statistical thinking, feature engineering, supervised learning, unsupervised learning, NLP, recommendation systems, time-series forecasting, visualization, and deployment.
